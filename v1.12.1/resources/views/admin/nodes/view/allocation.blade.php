...
@section('title')
    {{ $node->name }}: @lang('admin/node.view.allocation_tab.title')
@endsection

@section('content-header')
    <h1>{{ $node->name }}<small>@lang('admin/node.view.allocation_tab.header')</small></h1>
    <ol class="breadcrumb">
        ...
        <li class="active">@lang('admin/node.view.allocation')</li>
    </ol>
@endsection
    ...
    <div class="col-xs-12">
        <div class="nav-tabs-custom nav-tabs-floating">
            <ul class="nav nav-tabs">
                ...
                <li class="active"><a href="{{ route('admin.nodes.view.allocation', $node->id) }}">@lang('admin/node.view.allocation')</a></li>
    ...
    <div class="col-sm-8">
        <div class="box box-primary">
            <div class="box-header with-border">
                <h3 class="box-title">@lang('admin/node.view.allocation_tab.existing_allocations')</h3>
            </div>
            <div class="box-body table-responsive no-padding" style="overflow-x: visible">
                <table class="table table-hover" style="margin-bottom:0;">
                    <tr>
                        ...
                        <th>IP位址 <i class="fa fa-fw fa-minus-square" style="font-weight:normal;color:#d9534f;cursor:pointer;" data-toggle="modal" data-target="#allocationModal"></i></th>
                        <th>IP別名</th>
                        <th>@lang('strings.port')</th>
                        <th>@lang('strings.assigned_to')</th>
                        <th>
                            <div class="btn-group hidden-xs">
                                <button type="button" id="mass_actions" class="btn btn-sm btn-default dropdown-toggle disabled"
                                        data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">@lang('admin/node.view.allocation_tab.mass_actions') <span class="caret"></span>
                                </button>
                                <ul class="dropdown-menu dropdown-massactions">
                                    <li><a href="#" id="selective-deletion" data-action="selective-deletion">@lang('admin/node.view.allocation_tab.delete') <i class="fa fa-fw fa-trash-o"></i></a></li>
                                </ul>
                            </div>
                        </th>
                    </tr>
                    @foreach($node->allocations as $allocation)
                        <tr>
                            ...
                            <td class="col-sm-3 middle">
                                <input class="form-control input-sm" type="text" value="{{ $allocation->ip_alias }}" data-action="set-alias" data-id="{{ $allocation->id }}" placeholder="@lang('strings.none')" />
    ...
    <div class="col-sm-4">
        <form action="{{ route('admin.nodes.view.allocation', $node->id) }}" method="POST">
            <div class="box box-success">
                <div class="box-header with-border">
                    <h3 class="box-title">@lang('admin/node.view.allocation_tab.assign_new_allocations')</h3>
                </div>
                <div class="box-body">
                    <div class="form-group">
                        <label for="pAllocationIP" class="control-label">IP位址</label>
                        <div>
                            ...
                            <p class="text-muted small">@lang('admin/node.view.allocation_tab.allocation_ip_help')</p>
                        </div>
                    </div>
                    <div class="form-group">
                        <label for="pAllocationIP" class="control-label">IP別名</label>
                        <div>
                            <input type="text" id="pAllocationAlias" class="form-control" name="allocation_alias" placeholder="別名" />
                            <p class="text-muted small">@lang('admin/node.view.allocation_tab.allocation_alias_help')</p>
                        </div>
                    </div>
                    <div class="form-group">
                        <label for="pAllocationPorts" class="control-label">@lang('strings.ports')</label>
                        <div>
                            <select class="form-control" name="allocation_ports[]" id="pAllocationPorts" multiple></select>
                            <p class="text-muted small">@lang('admin/node.view.allocation_tab.allocation_ports_help')</p>
                        </div>
                    </div>
                </div>
                <div class="box-footer">
                    {!! csrf_field() !!}
                    <button type="submit" class="btn btn-success btn-sm pull-right">@lang('strings.submit')</button>
                </div>
            </div>
        </form>
    </div>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">@lang('admin/node.view.allocation_tab.delete_allocations_title')</h4>
            </div>
            <form action="{{ route('admin.nodes.view.allocation.removeBlock', $node->id) }}" method="POST">
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" class="btn btn-danger">@lang('admin/node.view.allocation_tab.delete')</button>
                </div>
            </form>
        </div>
    </div>
...
@section('footer-scripts')
    ...
    $('button[data-action="deallocate"]').click(function (event) {
        ...
        swal({
            title: '',
            text: '@lang('admin/node.view.allocation_tab.delete_confirm')',
            type: 'warning',
            ...
            $.ajax({
                ...
                swal({ type: 'success', title: '@lang('admin/node.view.allocation_tab.port_deleted')' });
            }).fail(function (jqXHR) {
                console.error(jqXHR);
                swal({
                    title: 'Whoops!',
    ...
    function deleteSelected() {
        ...
        if (selectedItems.length !== 0) {
            ...
            swal({
                type: 'warning',
                title: '',
                text: '@lang('admin/node.view.allocation_tab.delete_selected_confirm') ' + formattedItems + '?',
                html: true,
                showCancelButton: true,
                ...
                $.ajax({
                    ...
                    swal({
                        type: 'success',
                        title: '@lang('admin/node.view.allocation_tab.allocations_deleted')'
                    });
                }).fail(function (jqXHR) {
                    console.error(jqXHR);
                    swal({
