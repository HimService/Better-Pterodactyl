...
@section('content-header')
    <h1>@lang('admin/index.common.nodes')<small>@lang('admin/node.index.description')</small></h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('admin.index') }}">@lang('admin/index.admin')</a></li>
        <li class="active">@lang('admin/index.common.nodes')</li>
    </ol>
@endsection
    ...
    <div class="col-xs-12">
        <div class="box box-primary">
            <div class="box-header with-border">
                <h3 class="box-title">@lang('admin/node.index.list')</h3>
                <div class="box-tools search01">
                    <form action="{{ route('admin.nodes') }}" method="GET">
                        <div class="input-group input-group-sm">
                            <input type="text" name="filter[name]" class="form-control pull-right" value="{{ request()->input('filter.name') }}" placeholder="@lang('admin/node.index.search')">
                            <div class="input-group-btn">
                                <button type="submit" class="btn btn-default"><i class="fa fa-search"></i></button>
                                <a href="{{ route('admin.nodes.new') }}"><button type="button" class="btn btn-sm btn-primary" style="border-radius: 0 3px 3px 0;margin-left:-1px;">@lang('admin/node.index.create_new')</button></a>
                            </div>
                        </div>
                    </form>
                </div>
                ...
                <table class="table table-hover">
                    <tbody>
                        <tr>
                            ...
                            <th>@lang('strings.memory')</th>
                            <th>@lang('strings.disk')</th>
                            <th class="text-center">@lang('strings.servers')</th>
                            <th class="text-center">SSL</th>
                            <th class="text-center">@lang('admin/node.index.public')</th>
                        </tr>
    ...
    (function pingNodes() {
        $('td[data-action="ping"]').each(function(i, element) {
            $.ajax({
                ...
                var errorText = '{{ trans('admin/node.index.ping_error') }}';
                try {
