...
@section('title')
    @lang('admin/server.tabs.about') — {{ $server->name }}: @lang('admin/server.tabs.details')
@endsection

@section('content-header')
    <h1>{{ $server->name }}<small>@lang('admin/server.details.header_help')</small></h1>
    <ol class="breadcrumb">
        ...
        <li class="active">@lang('admin/server.tabs.details')</li>
    </ol>
@endsection
    ...
    <div class="col-xs-12">
        <div class="box box-primary">
            <div class="box-header with-border">
                <h3 class="box-title">@lang('admin/server.details.base_information')</h3>
            </div>
            <form action="{{ route('admin.servers.view.details', $server->id) }}" method="POST">
                <div class="box-body">
                    <div class="form-group">
                        <label for="name" class="control-label">@lang('strings.name') <span class="field-required"></span></label>
                        <input type="text" name="name" value="{{ old('name', $server->name) }}" class="form-control" />
                        <p class="text-muted small">@lang('admin/server.details.name_help')</p>
                    </div>
                    <div class="form-group">
                        <label for="external_id" class="control-label">@lang('admin/server.view.external_id')</label>
                        <input type="text" name="external_id" value="{{ old('external_id', $server->external_id) }}" class="form-control" />
                        <p class="text-muted small">@lang('admin/server.details.external_id_help')</p>
                    </div>
                    <div class="form-group">
                        <label for="pUserId" class="control-label">@lang('strings.owner') <span class="field-required"></span></label>
                        <select name="owner_id" class="form-control" id="pUserId">
                            <option value="{{ $server->owner_id }}" selected>{{ $server->user->email }}</option>
                        </select>
                        <p class="text-muted small">@lang('admin/server.details.owner_help')</p>
                    </div>
                    <div class="form-group">
                ...
                <div class="box-footer">
                    {!! csrf_field() !!}
                    {!! method_field('PATCH') !!}
                    <input type="submit" class="btn btn-sm btn-primary" value="@lang('admin/server.details.update_details')" />
                </div>
            </form>
        </div>
    </div>
