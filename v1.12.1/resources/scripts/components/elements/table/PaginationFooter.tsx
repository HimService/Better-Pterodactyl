    ...
    return (
        <div className={classNames('flex items-center justify-between my-2', className)}>
            <p className={'text-sm text-neutral-500'}>
                {t('global.showing', 'Showing')}&nbsp;
                <span className={'font-semibold text-neutral-400'}>
                    {Math.max(start, Math.min(pagination.total, 1))}
                </span>
                &nbsp;{t('global.to', 'to')}&nbsp;
                <span className={'font-semibold text-neutral-400'}>{end}</span> {t('global.of', 'of')}&nbsp;
                <span className={'font-semibold text-neutral-400'}>{pagination.total}</span> {t('global.results', 'results.')}
            </p>
