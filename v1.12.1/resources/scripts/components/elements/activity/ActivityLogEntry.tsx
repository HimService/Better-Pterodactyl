...
export default ({ activity, children }: Props) => {
    ...
    return (
        <div className={'grid grid-cols-10 py-4 border-b border-neutral-200 dark:border-neutral-800 last:rounded-b last:border-0 group'}>
            <div className={'hidden sm:flex sm:col-span-1 items-center justify-center select-none'}>
                <div className={'flex items-center w-10 h-10 rounded-full bg-neutral-200 dark:bg-neutral-800 overflow-hidden'}>
                    <Avatar name={actor?.uuid || 'system'} />
                </div>
            </div>
            <div className={'col-span-10 sm:col-span-9 flex'}>
                <div className={'flex-1 px-4 sm:px-0'}>
                    <div className={'flex items-center text-neutral-900 dark:text-gray-50'}>
                        <Tooltip placement={'top'} content={actor?.email || 'System User'}>
                            <span>{actor?.username || 'System'}</span>
                        </Tooltip>
                        <span className={'text-neutral-500 dark:text-gray-400'}>&nbsp;&mdash;&nbsp;</span>
                        <Link
