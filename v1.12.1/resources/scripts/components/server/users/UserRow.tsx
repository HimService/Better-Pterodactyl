...
export default ({ subuser }: Props) => {
    ...
    return (
        <GreyRowBox css={tw`mb-2`}>
            ...
            <div css={tw`w-10 h-10 rounded-full overflow-hidden hidden md:block shadow-sm`}>
            ...
            <div css={tw`ml-4`}>
                ...
                <p css={tw`text-2xs text-neutral-500 uppercase hidden md:block`}>{t('server.users.row.2fa_enabled', '2FA Enabled')}</p>
            </div>
            <div css={tw`ml-4 hidden md:block`}>
                <p css={tw`font-medium text-center`}>
                    {subuser.permissions.filter((permission) => permission !== 'websocket.connect').length}
                </p>
                <p css={tw`text-2xs text-neutral-500 uppercase`}>{t('server.users.row.permissions', 'Permissions')}</p>
            </div>
            {subuser.uuid !== uuid && (
                <>
                    <Can action={'user.update'}>
                        <button
                            type={'button'}
                            aria-label={t('server.users.row.edit_subuser', 'Edit subuser')}
