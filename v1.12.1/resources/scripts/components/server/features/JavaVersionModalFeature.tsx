...
const JavaVersionModalFeature = () => {
    ...
    return (
        ...
        >
            ...
            <h2 css={tw`text-2xl mb-4 text-neutral-100`}>{t('server.features.java_version.title', 'Unsupported Java Version')}</h2>
            <p css={tw`mt-4`}>
                {t('server.features.java_version.description_unsupported', 'This server is currently running an unsupported version of Java and cannot be started.')}
                <Can action={'startup.docker-image'}>
                    &nbsp;{t('server.features.java_version.description_select', 'Please select a supported version from the list below to continue starting the server.')}
                </Can>
            </p>
            <Can action={'startup.docker-image'}>
                <div css={tw`mt-4`}>
            ...
            <div css={tw`mt-8 flex flex-col sm:flex-row justify-end sm:space-x-4 space-y-4 sm:space-y-0`}>
                ...
                <Can action={'startup.docker-image'}>
                    <Button onClick={updateJava} css={tw`w-full sm:w-auto`}>
                        {t('server.features.java_version.update_button', 'Update Docker Image')}
                    </Button>
                </Can>
            </div>
        </Modal>
