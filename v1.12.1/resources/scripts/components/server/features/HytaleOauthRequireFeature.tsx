...
const HytaleOauthRequireFeature = () => {
    ...
    return (
        ...
        >
            ...
            <h2 css={tw`text-2xl mb-4 text-neutral-100`}>{t('server.features.hytale_oauth.title', 'Authentication Required')}</h2>
            <p css={tw`text-neutral-200`}>
                {t('server.features.hytale_oauth.description', 'You need to authenticate with your Hytale account to download or update server files. Please log in to continue.')}
            </p>
            <div css={tw`mt-8 sm:flex items-center justify-end`}>
                ...
                <Button onClick={handleLogin} css={tw`mt-4 sm:mt-0 sm:ml-4 w-full sm:w-auto`}>
                    {t('server.features.hytale_oauth.login_button', 'Log in')}
                </Button>
            </div>
        </Modal>
    );
