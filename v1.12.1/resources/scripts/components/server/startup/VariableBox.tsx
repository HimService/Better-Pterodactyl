...
const VariableBox = ({ variable }: Props) => {
    ...
    return (
        <TitledGreyBox
            title={
                <p className='text-sm uppercase'>
                    {!variable.isEditable && (
                        <span className='bg-[rgb(var(--bg-card-hover))] text-[rgb(var(--text-primary))] text-xs py-1 px-2 rounded-full mr-2 mb-1 border border-[rgb(var(--border-color))]'>{t('server.startup.read_only', 'Read Only')}</span>
                    )}
                    {variable.name}
                </p>
            }
        >
            ...
            <p className='mt-1 text-xs text-[rgb(var(--text-secondary))]'>{variable.description}</p>
        </TitledGreyBox>
    );
};
