        ...
        <GreyRowBox $hoverable={false} className={'flex-wrap md:flex-nowrap mt-2'}>
            <div className={'flex items-center w-full md:w-auto'}>
                ...
                <div className={'mr-4 flex-1 md:w-40'}>
                    ...
                    <Label>{allocation.alias ? t('server.network.hostname', 'Hostname') : t('server.network.ip', 'IP Address')}</Label>
                </div>
                <div className={'w-16 md:w-24 overflow-hidden'}>
                    <Code dark>{allocation.port}</Code>
                    <Label>{t('server.network.port', 'Port')}</Label>
                </div>
            </div>
            <div className={'mt-4 w-full md:mt-0 md:flex-1 md:w-auto'}>
                <InputSpinner visible={loading}>
                    <Textarea
                        className={'bg-neutral-100 dark:bg-neutral-800 hover:border-neutral-300 dark:hover:border-neutral-600 border-transparent text-neutral-800 dark:text-neutral-100'}
                        placeholder={t('server.network.notes', 'Notes')}
            ...
            <div className={'flex justify-end space-x-4 mt-4 w-full md:mt-0 md:w-48'}>
                {allocation.isDefault ? (
                    <Button size={Button.Sizes.Small} className={'!text-neutral-50 dark:!text-gray-50 !bg-blue-600'} disabled>
                        {t('server.network.primary', 'Primary')}
                    </Button>
                ) : (
                    <>
                        ...
                        <Can action={'allocation.update'}>
                            <Button.Text size={Button.Sizes.Small} onClick={setPrimaryAllocation}>
                                {t('server.network.make_primary', 'Make Primary')}
                            </Button.Text>
                        </Can>
                    </>
                )}
