                        ...
                        <div css={[tw`flex flex-wrap`, useMoveTerminology ? tw`items-center` : tw`items-end`]}>
                            <div css={tw`w-full sm:flex-1 sm:mr-4`}>
                                <Field
                                    ...
                                    id={'file_name'}
                                    name={'name'}
                                    label={t('server.files.file_name', 'File Name')}
                                    description={
                                        useMoveTerminology
                                            ? t('server.files.move_description', 'Enter the new name and directory of this file or folder, relative to the current directory.')
                                            : undefined
                                    }
                                    autoFocus
                                />
                            ...
                            <div css={tw`w-full sm:w-auto mt-4 sm:mt-0`}>
                                <Button css={tw`w-full`}>{useMoveTerminology ? t('global.move', 'Move') : t('global.rename', 'Rename')}</Button>
                            </div>
                        </div>
                        {useMoveTerminology && (
                            <p css={tw`text-xs mt-2 text-neutral-400`}>
                                <strong css={tw`text-neutral-200`}>{t('server.files.new_location', 'New location:')}</strong>
