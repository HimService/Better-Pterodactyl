...
export default () => {
    ...
    return (
        <ServerContentBlock title={t('server.files.header', 'File Manager')} showFlashKey={'files'}>
            <ErrorBoundary>
            ...
            {!files ? (
                ...
                <>
                    {!files.length ? (
                        <p css={tw`text-sm text-neutral-400 text-center`}>{t('server.files.empty_directory', 'This directory seems to be empty.')}</p>
                    ) : (
                        <CSSTransition classNames={'fade'} timeout={150} appear in>
                            <div>
                                <div className={'rounded-lg shadow-sm border border-[rgb(var(--border-color))] overflow-hidden'}>
                                    {files.length > 250 && (
                                        <div css={tw`bg-yellow-400 p-3`}>
                                            <p css={tw`text-yellow-900 text-sm text-center`}>
                                                {t('server.files.too_large', 'This directory is too large to display in the browser, limiting the output to the first 250 files.')}
                                            </p>
                                        </div>
                                    )}
