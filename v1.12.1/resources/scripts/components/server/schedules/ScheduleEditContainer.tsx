...
const CronBox = ({ title, value }: { title: string; value: string }) => (
    <div css={tw`bg-neutral-100 dark:bg-neutral-700 border border-neutral-200 dark:border-neutral-800 rounded p-3 transition-colors duration-300`}>
...
export default () => {
    ...
    return (
        <PageContentBlock title={t('server.schedules.header', '排程')}>
            <FlashMessageRender byKey={'schedules'} css={tw`mb-4`} />
            {!schedule || isLoading ? (
                <Spinner size={'large'} centered />
            ) : (
                <>
                    <ScheduleCronRow cron={schedule.cron} css={tw`sm:hidden bg-neutral-100 dark:bg-neutral-700 border border-neutral-200 dark:border-neutral-800 rounded mb-4 p-3 transition-colors duration-300`} />
                    <div css={tw`rounded shadow`}>
                        ...
                        >
                            <div css={tw`flex-1`}>
                                ...
                                <p css={tw`mt-1 text-sm text-neutral-200`}>
                                    最後執行時間：&nbsp;
                                    {schedule.lastRunAt ? (
                                        format(schedule.lastRunAt, "MM/dd '的' HH:mm")
                                    ) : (
                                        <span css={tw`text-neutral-300`}>無資料</span>
                                    )}
                                    <span css={tw`ml-4 pl-4 border-l-4 border-neutral-600 py-px`}>
                                        下次執行時間：&nbsp;
                                        {schedule.nextRunAt ? (
                                            format(schedule.nextRunAt, "MM/dd '的' HH:mm")
                                        ) : (
                                            <span css={tw`text-neutral-300`}>無資料</span>
                                        )}
                                    </span>
                        ...
                        <div css={tw`hidden sm:grid grid-cols-5 md:grid-cols-5 gap-4 mb-4 mt-4`}>
                            ...
                            <CronBox title={'星期'} value={schedule.cron.dayOfWeek} />
                        </div>
                        <div css={tw`bg-neutral-100 dark:bg-neutral-700 border-t border-neutral-200 dark:border-neutral-800 rounded-b transition-colors duration-300`}>
