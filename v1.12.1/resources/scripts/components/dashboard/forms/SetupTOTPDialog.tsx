        ...
        <form id={'enable-totp-form'} onSubmit={submit}>
            ...
            <CopyOnClick text={token?.secret}>
                <p className={'font-mono text-sm text-gray-100 text-center mt-2'}>
                    {token?.secret.match(/.{1,4}/g)!.join(' ') || t('global.loading', 'Loading...')}
                </p>
            </CopyOnClick>
            <p id={'totp-code-description'} className={'mt-6'}>
                {t('dashboard.account_overview.qr_code_help', 'Scan the QR code above using the two-step authentication app of your choice. Then, enter the 6-digit code generated into the field below.')}
            </p>
            <Input.Text
            ...
            <label htmlFor={'totp-password'} className={'block mt-3'}>
                {t('dashboard.account_overview.account_password', 'Account Password')}
            </label>
            <Input.Text
                variant={Input.Text.Variants.Loose}
                className={'mt-1'}
            ...
            <Dialog.Footer>
                ...
                <Tooltip
                    ...
                    content={
                        !token
                            ? t('dashboard.account_overview.waiting_for_qr', 'Waiting for QR code to load...')
                            : t('dashboard.account_overview.must_enter_code_and_password', 'You must enter the 6-digit code and your password to continue.')
                    }
                    delay={100}
                >
                    <Button
